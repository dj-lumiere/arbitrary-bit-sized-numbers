...
public:
    ...
    template<size_t MantSize = MantissaBits, size_t MantOffset = 0, typename MantMemoryPlace = MemoryPlace>
    ArbitraryUnsignedInt<MantSize, MantOffset, MantMemoryPlace> GetMantissa() const;
    
    template<size_t MantSize = MantissaBits + 1, size_t MantOffset = 0, typename MantMemoryPlace = MemoryPlace>
    ...
    static constexpr size_t ExponentBits() { return ExpBits; }
    static constexpr size_t GetMantissaBits() { return MantissaBits; }
    static constexpr size_t TotalBits() { return ExpBits + MantissaBits + 1; }
    static constexpr size_t Precision() { return MantissaBits + 1; }
    static constexpr size_t ExponentBias() { return (1 << (ExpBits - 1)) - 1; }
    static constexpr size_t MaxExponent() { return (1 << ExpBits) - 1; }
    static constexpr size_t MinExponent() { return 1 - ExponentBias(); }
    static constexpr size_t MaxNormalExponent() { return MaxExponent() - 1 - ExponentBias(); }
    ...
    // ===== STORAGE ACCESS =====
    const auto& GetStorage() const { return storage_; }
    auto& GetStorage() { return storage_; }

private:
    static constexpr size_t total_bits = ExpBits + MantissaBits + 1;
    static constexpr size_t data_bytes = (total_bits + 7) >> 3;

    using storage_type = typename MemoryPlace::template storage_type<data_bytes>;
...
template<size_t E, size_t M, typename MP>
ArbitraryFloat<E, M, MP> Hypot(const ArbitraryFloat<E, M, MP>& x, const ArbitraryFloat<E, M, MP>& y, const ArbitraryFloat<E, M, MP>& z);
